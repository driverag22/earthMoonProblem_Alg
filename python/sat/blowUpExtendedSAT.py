...
def solve_biplanar(edges, nodes):
    ...
    # order:    ab, a'b', a'b, ab'
    for i in range(0, len(edges) - 2 * 3 * 6, 4):
        group = edges[i:i+4]
        print(group)
    ...
    for i in range(len(edges) - 2 * 3 * 6, len(edges), 6):
        group = edges[i:i+6]
        print(group)
        vars_group = [edge_to_var(e, edge_to_var_map) for e in group]
        # corresponds to:
        ...
        for i1 in range(6):
            for i2 in range(i1+1, 6):
                ...
                if frozenset((a, b)) in blown_pairs:
                    ...
                    continue
                # only if there is an edge u-v in the blown graph
                key = (a, b) if (a, b) in edge_to_var_map else (a, b)
                if key in edge_to_var_map:
                    ab = edge_to_var_map[key]
                    av = vars_group[i1]  # a v, v interior/extra vertex
                    bv = vars_group[i2]  # b v, v interior/extra vertex
                    # av and bv => a
                    #   ===
                    # [not (av and bv)] or (ab) = (not av) or (not bv) or (ab)
                    solver.add_clause([-av, -bv, ab])
                    # (not av) and (not bv) => (not ab)
                    #   ===
                    # av or bv or (not ab)
                    solver.add_clause([av, bv, -ab])
